...
class InvoicesbatchesController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('client.invoicesbatches.index', $data);
    }
    ...
    public function delete($batch_id)
    {
        ...
        if (count($invoicesbatches) < 1) {
            ...
            return redirect()->route('client.invoicesbatches.index');
        }
        if ($invoicesbatches->client_id != Auth::user()->client->id) {
            abort(403);
        }
    ...
    public function printview($batch_id)
    {
        ...
        if ($batch->client_id != Auth::user()->client->id) {
            abort(403);
        }
        $invoices_batch = unserialize($batch->invoice_id);
        ...
        return view('client.invoicesbatches.printview', $data);
    }
    ...
    public function payment($batch_id)
    {
        ...
        $data = [
            ...
            'work_id' => 0,
            'attach_id' => 0,
            'batch_date' => date('Y-m-d', strtotime($invoicesbatches->created_at)),
            'batch_id' => $batch_id,
            'client_company' => Auth::user()->client->company_name,

        ];

        return view('client.invoicesbatches.payment', $data);
    }
    ...
    public function submitpayment(Request $request)
    {
        ...
        if (! $client->payeezy_value) {
            return view('client.invoicesbatches.setupcard');
        }
        ...
        if ($result_data->transaction_status == 'approved') {
            ...
            return view('client.invoicesbatches.unpaid');
        }
    }
