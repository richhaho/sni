...
class PdfPagesController extends Controller
{
    ...
    public function deleteRegenerate($id)
    {
        ...
        Session::flash('message', 'Attachments(PDF) were deleted.');
        return redirect()->route('client.notices.document', $id);
    }
    ...
    public function askreset($id) {
        ...
        return view('client.pdf.document.reset',$data);
    }
    ...
    public function doreset(Request $request,$id) { 
        ...
        if($request->reset == "yes") {
            ...
            return redirect()->route('client.notices.document', $id)->with('from',$request->backurl);
        } else {
    ...
    public function docancel(Request $request, $id) {
        ...
        if(strlen($request->backurl)) {
            if($request->backurl == url(route('client.notices.reset.ask',$id)))  {
                 return redirect()->route('client.notices.index');
            }
        }
        return redirect()->route('client.notices.index');
    ...
    public function document(Request $request,$id)
    {
        ...
        if (!isset($request['fromAttachPDF'])){
            if (count($work->pdf_pages)>0) {
                return redirect()->route('client.notices.reset.ask',$id)->with(['from' => $backurl]);
            }
        }
        $job = $work->job;
        ...
        if ($bond) {$bond_number=$bond->id;}else{$bond_number='';}
    ...
    //============ For NOC ============
        ...
        if (1==0) {
            ...
            if ($work->type =='amended-notice-to-owner') {
                if (isset($wo_nto->id)) {$wo_nto_number=$wo_nto->id;}else{$wo_nto_number="";}
                $antopage = [
                    'type' => $work->type,
                    'client_company_name' => $client_company_name,
                    'nto_date' => $today,
            ...
            if ($work->type =='rescission-letter') {
                if (isset($wo_nto->number)) {$wo_nto_number=$wo_nto->number;}else{$wo_nto_number="";} 
                $rlpage = [
                    'type' => $work->type,
                    'job_type' => $job_type,
                    'barcode' => '',
        ...
        return view('client.pdf.document',$data);
    ...
    public function update(Request $request,$id)
    {
        ...
        if ($fields['type'] == 'amend-claim-of-lien') {
            return view('client.pdf.acol', unserialize($page->fields));
        }
        if ($fields['type'] == 'amended-notice-to-owner') {
            return view('client.pdf.anto', unserialize($page->fields));
        }
        if ($fields['type'] == 'amended-notice-to-owner-back') {
            return view('client.pdf.antoback', unserialize($page->fields));
        }

        if ($fields['type'] == 'claim-of-lien') {
            return view('client.pdf.col', unserialize($page->fields));
        }
        if ($fields['type'] == 'conditional-waiver-and-release-of-lien-upon-final-payment') {
            return view('client.pdf.cwarolufp', unserialize($page->fields));
        }
        
        if ($fields['type'] == 'conditional-waiver-and-release-of-lien-upon-progress-payment') {
            return view('client.pdf.cwarolupp', unserialize($page->fields));
        }
        
        if ($fields['type'] == 'contractors-final-payment-affidavit') {
            return view('client.pdf.cfpa', unserialize($page->fields));
        }
        
        if ($fields['type'] == 'notice-of-bond') {
            return view('client.pdf.nob', unserialize($page->fields));
        }
        if ($fields['type'] == 'notice-of-commencement') {
            return view('client.pdf.noc', unserialize($page->fields));
        }
        if ($fields['type'] == 'notice-of-termination') {
            return view('client.pdf.not', unserialize($page->fields));
        }
        if ($fields['type'] == 'notice-to-owner') {
            return view('client.pdf.nto', unserialize($page->fields));
        }
        if ($fields['type'] == 'notice-to-owner-back') {
            return view('client.pdf.ntoback', unserialize($page->fields));
        }
        if ($fields['type'] == 'notice-of-contest-of-claim-against-payment-bond') {
            return view('client.pdf.nococapb', unserialize($page->fields));
        }
        
        if ($fields['type'] == 'notice-of-contest-of-lien') {
            return view('client.pdf.nocol', unserialize($page->fields));
        }
        if ($fields['type'] == 'notice-of-non-payment') {
            return view('client.pdf.nonp', unserialize($page->fields));
        }
        if ($fields['type'] == 'notice-of-nonpayment-for-bonded-private-jobs-statutes-713') {
            return view('client.pdf.nonp713', unserialize($page->fields));
        }
        if ($fields['type'] == 'notice-of-nonpayment-for-government-jobs-statutes-255') {
            return view('client.pdf.nonp255', unserialize($page->fields));
        }
        if ($fields['type'] == 'out-of-state-nto-preliminary-notice-of-lien-rights') {
            return view('client.pdf.pnolr', unserialize($page->fields));
        }
        if ($fields['type'] == 'rescission-letter') {
            return view('client.pdf.rl', unserialize($page->fields));
        }
        if ($fields['type'] == 'notice-of-nonpayment-with-intent-to-lien-andor-foreclose') {
            return view('client.pdf.nonwitlaf', unserialize($page->fields));
        }
        
        if ($fields['type'] == 'partial-satisfaction-of-lien') {
            return view('client.pdf.psol', unserialize($page->fields));
        }
        if ($fields['type'] == 'satisfaction-of-lien') {
            return view('client.pdf.sol', unserialize($page->fields));
        }
        if ($fields['type'] == 'sworn-statement-of-account') {
            return view('client.pdf.ssoa', unserialize($page->fields));
        }
        
        if ($fields['type'] == 'waiver-and-release-of-lien-upon-final-payment') {
            return view('client.pdf.warolufp', unserialize($page->fields));
        }
        
        if ($fields['type'] == 'waiver-and-release-of-lien-upon-progress-payment') {
            return view('client.pdf.warolupp', unserialize($page->fields));
        }
        
        if ($fields['type'] == 'waiver-of-right-to-claim-against-bond-final-payment') {
            return view('client.pdf.wortcabfp', unserialize($page->fields));
        }
        
        if ($fields['type'] == 'waiver-of-right-to-claim-against-bond-progress-payment') {
            return view('client.pdf.wortcabpp', unserialize($page->fields));
        }
    }
        ...
        if ($request->generate == "preview") {
            foreach($wo->pdf_pages as $xpage) {
                ...
                if ($xpage->type == 'notice-to-owner') {
                    ...
                    $data['document'] = 'nto';
                    $pdf = PDF::loadView('client.pdf.pdf-document', $data)->setPaper('Letter');
                }
    ...
    public function AttachPDF(Request $request,$id){
        ...
        if ($request['loading']=='start'){
            ...
            return view('client.pdf.attachpdf',$data);
        }
        //////////////////////////////////////////////////////////
        ...
        return view('client.pdf.mailparties',$data);
    }
...
if ($request['attachedFile']){
  ...
  if ($request['PlaceTpye']=='Use in Place of Notice'){
            ...
            $rp->delete();
                ...
                foreach($request->party_id as $xid) {
                    ...
                    if($xmailing_type[$xid] <> 'none') {
                        ...
                        } else if (isset($xalso_emails[$xid])) {
                    ...
                    if (count($pdf_pages)>0 ) {
                        ...
                        if ($pageCount>$max_page_length && ($recipient->mailing_type != 'none' && $recipient->mailing_type != 'other-mail' )) $max_page_length = $pageCount;
            ...
            if($doit) {
                ...
                foreach($work->recipients as $xrx ) {
                    ...
                    if ($work->service=='self') {
                        ...
                        if (count($sslines)>0) $tlines = $sslines;
                    }
                    foreach ($tlines as $xln) {
        ...
        return view('client.pdf.review',$data); 
  }else{
///////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////
 $this->validate($request,[
    ...
    public function paid($payment_id) {
        ...
        return view('client.pdf.paid',$data);
    }
    ...
    public function unpaid($payment_id) {
        ...
        return view('client.pdf.unpaid',$data);
    }
}