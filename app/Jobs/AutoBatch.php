...
class AutoBatch implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        foreach($clients as $client) {
            if ($client->autobatch!=1) continue;
            ...
            $past_total=0;$past_postage=0;
            foreach($client->batch_invoices()->where('payed_at',null)->where('id','!=',$batch->id)->get() as $un_batch){
                $past_total+=(double)filter_var($un_batch->total_amount, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
