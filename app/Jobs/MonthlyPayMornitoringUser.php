...
class MonthlyPayMornitoringUser implements ShouldQueue
{
    ...
    {
        // Mail::raw('MonthlyPayMornitoringUser job started.', function($message)
        // {
        //     $message->subject('MonthlyPayMornitoringUser Start');
        //     $message->from('no-reply@sunshinenotice.com', 'SunshineNotice');
        //     $message->to('jwatson@ironrocksoftware.com');
        // });
        ////////////////////////////////////////////////////////////////////////
        $nowMonth = date('y-m');
        $clients = Client::where('deleted_at', null)->where('is_monitoring_user', 1)->where('monthly_payment', '>', 0)->get();
        foreach($clients as $client) {
            ...
            try {
                ...
                $payload = [
                    ...
                    'method'=> 'token',
                    'amount'=> number_format($client->monthly_payment,2,'',''),
                    'currency_code'=> 'USD',
                    'token'=> [
                        'token_type'=> 'FDToken',
                        'token_data'=> [
                ...
                if ( $result_data->transaction_status == "approved") {
                    ...
                    $payment->amount = $client->monthly_payment;
                    ...
                    $payment->service_type = 'monthly-payment';
                    ...
                    if(json_encode(unserialize($client->override_payment))!="false" && json_encode(unserialize($client->override_payment))!="null"){
                        Mail::to(unserialize($client->override_payment))->send(new MonthlyPaymentMade($payment,$client));
                    }else{
                        $mailto = array();
                        $users = $client->activeusers;
                        foreach ($users as $user) {
                        ...
                        if (count($mailto) > 0) {
                            Mail::to($mailto)->send(new MonthlyPaymentMade($payment,$client));
                        }
                    }
                }
        ...
        ////////////////////////////////////////////////////////////////////////
        // Mail::raw('MonthlyPayMornitoringUser job completed.', function($message)
        // {
        //     $message->subject('MonthlyPayMornitoringUser End');
        //     $message->from('no-reply@sunshinenotice.com', 'SunshineNotice');
        //     $message->to('jwatson@ironrocksoftware.com');
        // });
    }
