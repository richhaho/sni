                        ...
                        <h1 class="page-header">Clients  
                           ...
                           <div class="col-xs-12" id="filters-form">
                            {!! Form::open(['route' => 'clients.setfilter', 'class'=>'form-inline'])!!}
                                  ...
                                  <div class="form-group">
                                    <label for="search">Approve Status: </label>
                                    {!!  Form::select('approval_status',$approval_status,old('approval_status',$filter_status), ['class' => 'form-control','data-toggle'=>'tooltip', 'data-placement'=>'top', 'title'=>'']) !!}
                                  </div>
                                  <div class="form-group">
                                    <label for="search">  Status: </label>
                                    {!!  Form::select('client_status',$client_status,old('client_status',$filter_client_status), ['class' => 'form-control','data-toggle'=>'tooltip', 'data-placement'=>'top', 'title'=>'']) !!}
                                  </div>
                                
                            <button class="btn btn-success" type="submit" ><i class="fa fa-search"></i> Search</button>
                             <a href="{{ route('clients.resetfilter') }}" class="btn btn-danger">Clear</a>
                            {!! Form::close() !!}
                           </div>
                            ...
                            <tbody>
                                ...
                                <tr class="{{ ($client->status == 3 ) ? 'client-disabled' : '' }}">
                                    ...
                                    <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                      ...
                                      @component('admin.clients.components.deletemodal')
                                        @slot('id') 
                                            {{ $client->id }}
                                        @endslot
                                        @slot('client_name') 
