...
@section('content')
    {!! Form::open(['route' => ['users.update',$user->id], 'method'=> 'PUT','autocomplete' => 'off']) !!}
        ...
        <div id="top-wrapper" >
            ...
            <div  class="col-xs-12">
                <h1 class="page-header">Edit Admin User {{ $user->name }}
                    <div class="pull-right">
                        <button class="btn btn-success " type="submit"> <i class="fa fa-floppy-o"></i> Save</button>
                        <a class="btn btn-danger " href="{{ route('users.index')}}"><i class="fa fa-times-circle"></i> Cancel</a> &nbsp;&nbsp;
                    </div>
                </h1>       
            </div>
            </div>
                        ...
                        <div class="panel panel-default">
                            ...
                            <div class="panel-body">
                                ...
                                <div class="col-xs-12 col-md-12 form-group">
                                    <label>Role:</label>
                                    {{ Form::select('role_id',$roles,old('role_id',$user->roles()->first()->id),['class' => 'form-control'])}}
                                </div>
                                </div>
                                <div class="row">
                                ...
                                <div class="col-xs-12  col-md-3 form-group restrict-group {{$user->roles()->first()->id==4 ? '' : 'hidden'}}">
                                    <label>Researcher Role:</label>
                                    {!!  Form::select('restricted',['0' =>'Unrestricted', '1'=> 'Restricted'],old("restricted",$user->restricted), ['class' => 'form-control']) !!}
                                </div>
                                </div>
...
$(function () {
  ...
  $('select[name="role_id"]').change(function() {
    if ($('select[name="role_id"]').val() == '4') {
        $('.restrict-group').removeClass('hidden');
    } else {
        $('.restrict-group').addClass('hidden');
    }
  });
