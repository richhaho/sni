...
<div class="row">
    <div class="col-xs-12" style="margin-bottom: 10px">
        ...
        @component('admin.jobs.reminders.components.createmodal')
            @slot('job_id') 
                {{ $job->id }}
            @endslot
        @endcomponent
                ...
                <tr>
                    ...
                    <td>
                        <div class="btn-group pull-right dropup">
                            ...
                            @component('admin.jobs.reminders.components.editmodal')
                                @slot('id') 
                                    {{ $reminder->id }}
                                @endslot
                                @slot('job_id') 
                                ...
                                @endslot
                            @endcomponent
                            @component('admin.jobs.reminders.components.deletemodal')
                                @slot('id') 
                                    {{ $reminder->id }}
                                @endslot
                                @slot('job_id') 
