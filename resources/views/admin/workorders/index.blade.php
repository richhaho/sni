...
@section('content')
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-xs-12">
                        <h1 class="page-header">Full Service Work Orders
                        ...
                        <div class="col-xs-12" id="filters-form">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    {!! Form::text('job_address',session('work_order_filter.job_address'),['class'=>'form-control job_address_filter'])!!}
                                </div>
                                <div class="form-group">
                            ...
                            <button class="btn btn-success" type="submit" ><i class="fa fa-filter"></i> Enter</button>
                             <a href="{{ route('workorders.resetfilter') }}" class="btn btn-danger">Clear</a>
                             </div>
                            {!! Form::close() !!}
                        ...
                        <table class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th>Due Date</th>
                                    <th>Status</th>
                                    @if (session('work_order_filter.work_status')=="phone calls")
                            ...
                            <tbody>
                                ...
                                if ($work->type=='notice-to-owner' || $work->type=='amended-notice-to-owner'){
                                    $diff=(strtotime(\Carbon\Carbon::now())-strtotime($work->job->started_at))/3600/24;
                                    if(array_search($work->status, $status)===false  && $diff>=35 && $diff<45){
                                        $style='background-color: #FFE4E1';
                                    }
                                }
                                if ($work->type=='claim-of-lien' || $work->type=='amend-claim-of-lien' ||  $work->type=='notice-of-non-payment'){
                                    $diff=(strtotime(\Carbon\Carbon::now())-strtotime($work->job->last_day))/3600/24;
                                    if(array_search($work->status, $status)===false && $diff>=60){
                                        $style='background-color: #FFE4E1';
                                    }
                                }

                                $diff=(strtotime(\Carbon\Carbon::now())-strtotime($work->created_at))/3600/24;
                                if(array_search($work->status, $status)===false && $diff>=15){
                                    if ($style=='') $style='background-color: #F5F5DC';
                                }
                                ?>
                                <tr style="{{$style}}">
                                    <td> {{ $work->number }}</td>
