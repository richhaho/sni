                            ...
                            <div class="panel panel-default">
                                ...
                                <div class="panel-body">
                                    ...
                                    <div id="new-contact" class="{{ Session::exists('_old_input') ? array_key_exists("first_name",Session::get('_old_input')) ? '' : 'hidden' : 'hidden' }}">
                                        ...
                                        @include('admin.jobparties.dynamicforms.newcontact')
                                    </div>

                                    <div id="new-hotcontact" class="{{ Session::exists('_old_input') ? array_key_exists("first_name",Session::get('_old_input')) ? '' : 'hidden' : 'hidden' }}">
                                        
                                        @include('admin.jobparties.dynamicforms.newhotcontact')
                                    </div>
                                        ...
                                        <div class="panel panel-default">
                                            ...
                                            <div class="panel-body">
                                               @foreach($job->parties()->ofType($type_key)->get() as $jobparty) 
                                                    ...
                                                    @include('admin.jobparties.components.contacticon')
...
$('.add_job_party').click(function(){
    ...
    $('.add_job_party').css('pointer-events','none');
}); 
...
$(function () {
   ...
   $("#contact_id").select2({
        ...
        ajax: {
            url: '{{url("/admin/jobs")}}/'+ job_id + '/contacts',
            dataType: 'json',
            type: "GET",
            delay: 50,
   ...
   $("#contact_id").on('select2:open',function() {
       if (!button_added ) {
       $(".select2-dropdown").append('<div class="row select2-new-button"><div class="col-xs-5 col-xs-offset-3"><a id="new-hotcontact-button" href="#" class="btn btn-success btn-xs btn-block"><i class="fa fa-plus"></i> New Hot Contact</a></div><div class="col-xs-4"><a id="new-contact-button" href="#" class="btn btn-success btn-xs btn-block"><i class="fa fa-plus"></i> New Contact</a></div></div>');
       button_added =true;
       }
   });  
  ...
  function updatetype() {
   ...
   $('body').on('click','#new-contact-button', function(){
       ...
       $('#new-hotcontact').addClass('hidden');
       $('#new-hotcontact').find('select,input').prop('disabled',true);


      $('#contact_id').prop('disabled',true);
        $(".choose-company").addClass('hidden').find('select').prop('disabled',true);
        $("#new-contact .new-company").removeClass('hidden').find('input').prop('disabled',false);

       $('#cancel-new-contact-button').toggleClass('hidden');
       $('#box').addClass('hidden');
   });
   $('body').on('click','#new-hotcontact-button', function(){
       $('#contact_id').select2("close");
       $('#new-hotcontact').removeClass('hidden');
       $('#new-hotcontact').find('select,input').prop('disabled',false);
       $('#new-contact').addClass('hidden');
       $('#new-contact').find('select,input').prop('disabled',true);

      $('#contact_id').prop('disabled',true);
        $(".choose-company").addClass('hidden').find('select').prop('disabled',true);
        $("#new-hotcontact .new-company").removeClass('hidden').find('input').prop('disabled',false);
    ...
    $('body').on('click','#choose-company-button', function() {
        ...
        if ($("#new-contact").attr('class')!='hidden'){

          $("#new-contact .choose-company").removeClass('hidden').find('select').prop('disabled',false);

          $("#new-contact .new-company").addClass('hidden').find('input').prop('disabled',true);
        }

        if ($("#new-hotcontact").attr('class')!='hidden'){

          $("#new-hotcontact .choose-company").removeClass('hidden').find('select').prop('disabled',false);

          $("#new-hotcontact .new-company").addClass('hidden').find('input').prop('disabled',true);
        }
    ...
    $('body').on('click','#new-company-button', function() {
      if ($("#new-contact").attr('class')!='hidden'){  
        $("#new-contact .choose-company").addClass('hidden').find('select').prop('disabled',true);
        $("#new-contact .new-company").removeClass('hidden').find('input').prop('disabled',false);
      }

      if ($("#new-hotcontact").attr('class')!='hidden'){  
        $("#new-hotcontact .choose-company").addClass('hidden').find('select').prop('disabled',true);
        $("#new-hotcontact .new-company").removeClass('hidden').find('input').prop('disabled',false);
      }
    });
   ...
   leasetype();
    ...
    var countries = new Bloodhound({
            ...
            prefetch:  { url: '{{ route('list.countries') }}' , cache: false }
              ...
              if($('#new-hotcontact').hasClass('hidden')) {
                  $('#new-hotcontact').find('select,input').prop('disabled',true);
                  $('#box').find('select').prop('disabled',false);
                  $('#cancel-new-contact-button').addClass('hidden');
              } else {
              ...
              @if(Session::get('parties.newcontact')=='hot')
                $('#new-contact').addClass('hidden');
                $('#new-contact').find('select,input').prop('disabled',true);
              @endif
              @if(Session::get('parties.newcontact')=='new')
                $('#new-hotcontact').addClass('hidden');
                $('#new-hotcontact').find('select,input').prop('disabled',true);
              @endif
