                ...
                <div class="row">
                    <div class="page-header col-xs-12">
                       ...
                       <div class="col-xs-12" id="filters-form">
                            {!! Form::open(['route' => 'mailinghistory.setfilter2', 'class'=>'form-inline'])!!}
                                @if(count($clients) > 0)
                                <div class="form-group">
                                    <label for="mailing_type_filter">Client: </label>
                                    {!! Form::select('client_filter',$clients,session('mailinghistory_filter.client'),['class'=>'form-control','id'=>'client_filter'])!!}
                                </div>
                                @endif
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                 ...
                                 <div class="form-group">
                                    ...
                                    {!! Form::select('job',$jobs,session('mailinghistory_filter.job'),['class'=>'form-control','id'=>'job_filter', 'style' => 'width: 400px'])!!}
                                </div>
                                ...
                                <div class="form-group">
                                    <label for="job_type_filter"> Mailing Date: </label>
                            ...
                            <button class="btn btn-success" type="submit" ><i class="fa fa-filter"></i> Enter</button>
                             <a href="{{ route('mailinghistory.resetfilter2') }}" class="btn btn-danger">Clear</a>
                            {!! Form::close() !!}
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                @if ($mailing->attachable)
                                <tr>
                                     <td class="address">
                                        {{$mailing->recipient->firm_name}}<br />
                                        <span >{!! nl2br($mailing->recipient->address) !!}</span>
                                    </td>
                                    <td style="word-break: break-all;">
                                        @if(strlen($mailing->recipient->barcode))
                                            {{ $mailing->recipient->barcode }}
                                        @elseif(strlen($mailing->recipient->mailing_number))
                                            {{ $mailing->recipient->mailing_number }}
                                        @else
                                            N/A
                                        @endif
                                    </td>
                                    <td>{{  $mailing->printed_at ? $mailing->printed_at->format('m-d-Y') : 'N/A'}}</td>  
                                    ...
                                    <td>
                                        <?php
                                        $batch_detail=\App\BatchDetail::where('attachment_id',$mailing->attachment_id)->first();
                                        $batch=$batch_detail->batch;
                                        if ($batch->manifest_file){
                                        ?>
                                        <a href="{{ route('mailing.downloadmanifest',$batch->id)}}" class="btn btn-success btn-xs"><i class="fa fa-download"></i> Download </a>
                                    <?php } ?>
                                    </td>
                                    <td>
                                        @if( $mailing->resent <= 0)
                                       
                                            @component('admin.mailinghistory.components.deletemodal2')
                                            @slot('id') 
                                                {{ $mailing->attachment_id }}
                                            @endslot
                                            @slot('button_caption') 
                                                {{ $mailing->resent == 0  ? 'Resend' :'Cancel Resend' }}
                                            @endslot
                                            @endcomponent
                                        @endif
                                    </td>
