...
<div id="page">
    ...
    <p class="warning text-justify">THIS LEGAL DOCUMENT REFLECTS THAT A CONSTRUCTION LIEN HAS 
        BEEN PLACED ON THE REAL PROPERTY LISTED HEREIN. UNLESS THE OWNER OF SUCH 
        PROPERTY TAKES ACTION TO SHORTEN THE TIME PERIOD, THIS LIEN MAY REMAIN 
        VALID FOR ONE YEAR FROM THE DATE OF RECORDING, AND SHALL EXPIRE AND 
        BECOME NULL AND VOID THEREAFTER UNLESS LEGAL PROCEEDINGS HAVE BEEN 
        COMMENCED TO FORECLOSE OR TO DISCHARGE THIS LIEN.</p>
    <h3 class="text-center">CLAIM OF LIEN</h3>
    ...
    <p class="text-justify">BEFORE ME, the undersigned authority, personally appeared {{ $client_name }}, who, 
        ...
        of the lienor herein, {{$client_company_name}}, {{ preg_replace('/\<br(\s*)?\/?\>/i', " ", $client_address) }} 
        and that in accordance with a contract with {{ $customer_name }}. The
        Lienor furnished labor, services, or materials consisting of
        {!! $materials !!} on the following described real property 
        in {{ $job_county }} COUNTY, Florida:
    </p>
    <div class="row">
    ...
    <p class="text-justify" style="margin-top: 10px;">
       ...
       of ${{number_format($unpaid_balance,2)}} @if ($interest_amount > 0) plus interest in the amount of ${{number_format($interest_amount,2)}} @else (including interest if contract permits) @endif
       ...
       @else
        the outstanding balance is due thru @if($thru_date) {{$thru_date}}, @else __/__/2018, @endif as this work continues to be performed on the project and should the amount claimed herein change, we reserve the option to amend within 90 days from the last day of work
       @endif
       @if(strlen($nto_date))
       ; and <strong>(if the lien is claimed by one not in privity with the owner)</strong> that the Lienor served Notice to Owner to all parties involved on {{ date_format(date_create($nto_date),'F jS, Y') }} <!-- by Certified and/or Regular Mail --> as required by Florida Statutes. 
       @else
       .
       @endif
       <br>
       The negligent inclusion or omission of any information in the Claim of Lien, which has not prejudiced the owner, does not defeat an otherwise valid lien.
    ...
    <p>
        WITNESS my hand and official seal in the County and State last aforesaid this ____ day of {{$month ? ucfirst($month) :'______________________'}}, {{ isset($year) ? $year :'' }}.
    </p>
   <div class="row">
       <table style="width: 100%">
           <tr>
