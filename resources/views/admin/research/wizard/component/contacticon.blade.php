<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ">
    ...
    @else
    <p><a href="{{ route('parties.downloadbond',[$jobparty->job_id,$jobparty->id]) }}"><i class="fa fa-file"></i> {{$jobparty->bond_pdf_filename}}</a>({{ number_format($jobparty->bond_pdf_filename_size,2)}}KB)</p>
    @endif
    ...
    @endif
        ...
        <div class="controls">
            @component('admin.research.wizard.component.deletemodal')
                @slot('id') 
                    {{ $jobparty->id }}
                @endslot
                @slot('job_party_name') 
                ...
                @endslot
            @endcomponent 
            <a href="{{ route('research.wizard.parties.edit',[ $jobparty->job_id, $jobparty->id])}}?from=step{{$step}}" class="btn btn-success btn-xs" role="button" data-toggle="tooltip" data-placement="right" title="Edit"><i class="fa fa-pencil"></i> Edit</a>
            @if($step!='2')
            @component('admin.research.wizard.component.copyparty')
                @slot('id') 
                    {{ $jobparty->id }}
                @endslot
                @slot('job_id') 
