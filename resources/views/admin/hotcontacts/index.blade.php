                ...
                <div class="row">
                    <div class="col-xs-12">
                        <h1 class="page-header">Contacts HotList
                            ...
                            <a class="btn btn-success btn-save" href="{{ route('hotcontacts.create')}}"><i class="fa fa-plus"></i> Add Contact</a> 
                            </div>
                        </h1>
                        <br>
                        <div class="col-xs-12" id="filters-form">
                            {!! Form::open(['route' => 'hotcontacts.setfilter', 'class'=>'form-inline'])!!}
                                  <div class="form-group">
                                    <label for="search">Search: </label>
                                    {!! Form::text('search',session('hot_filter.search'),['class'=>'form-control','placeholder' => 'Search...'])!!}
                                  </div>
                                  
                            <button class="btn btn-success" type="submit" ><i class="fa fa-search"></i> Search</button>
                            <a href="{{ route('hotcontacts.resetfilter') }}" class="btn btn-danger">Clear</a>  
                            {!! Form::close() !!}
                        <br>
                        </div>
                                      ...
                                      <div class="panel-heading" role="tab" id="heading{{$entity->id}}">
                                        ...
                                        <div class="panel-title ">
                                          <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapse{{$entity->id}}" aria-expanded="true" aria-controls="collapse{{$entity->id}}">
                                            ...
                                            <div class="pull-right">
                                                @component('admin.hotcontacts.components.deletemodal')
                                                    @slot('id') 
                                                        {{ $entity->id }}
                                                    @endslot
                                                    @slot('firm_name') 
                                        ...
                                        <div class="panel-body">
                                            <div class="row same-height" >
                                                @foreach($entity->contacts()->orderBy('primary','desc')->orderBy('last_name','asc')->get() as $contact) 
                                                    ...
                                                    @include('admin.hotcontacts.components.contacticon')
                        ...
                        @if(count($entities) > 0 || count($new_entity)>0)
                            <div class="col-xs-12">
                                <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
