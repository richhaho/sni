            ...
            <div  class="col-xs-12">
                <h1 class="page-header">Edit Invoice {{$invoice->number}}
                    <div class="pull-right">
                        ...
                        <button class="btn btn-success btn-save" type="button" disabled> <i class="fa fa-floppy-o"></i> Save</button>
                        @else
                        <button class="btn btn-success btn-save" type="button"> <i class="fa fa-floppy-o"></i> Save</button>
                        @endif
                ...
                <div class="row">
                    ...
                    <div class="col-xs-12">
                        <div class="panel panel-default">
                                ...
                                <table class="table lines">
                                    ...
                                    <tr class="existent-line-{{$line->id}}">
                                        <td>
                                            ...
                                            <div class="col-xs-2 form-group">
                                                <label>Price</label>
                                                <input type="number" data-id="{{$line->id}}" name="price[{{$line->id}}]" value="{{ old("price." . $line->id, $line->price)}}" placeholder="price"  step="0.01"  class="form-control price noucase">
                                            </div>
                                             <div class="col-xs-2 form-group">
                                                <label>Amount</label>
                                                <input disabled type="number" data-id="{{$line->id}}" name="amount[{{$line->id}}]" value="{{ old("amount." . $line->id, $line->amount)}}" placeholder="price"  step="0.01" class="form-control amount">
                                            </div>
                                    ...
                                    <tr class="new-line-{{$key}}"><td>
                                       ...
                                       <div class="col-xs-2 form-group">
                                           <label>Price</label>
                                           <input type="number" data-newid="@i"  name="new_price[{{$key}}]" value="{{old('new_price')[$key]}}" placeholder="price"  step="0.01" class="form-control new_price noucase">
                                       </div>
                                        <div class="col-xs-2 form-group">
                                            <label>Amount</label>
                                            <input disabled data-newid="@i"  type="number" name="new_amount[{{$line->id}}]" value="{{ old("new_amount." . $line->id, $line->amount)}}" placeholder="Amount"  step="0.01" class="form-control new_amount">
                                        </div>
                                       <div class="col-xs-1" form-group">
                                           <label>&nbsp;&nbsp;&nbsp;</label>
                            ...
                            <div class="negative col-xs-4 col-xs-offset-7" style="display: none;color: red">
                              <p>Invoice Total must be greater than 0.</p>
                            </div> 
                        </div>
                    </div>
                    <!-- /.col-lg-12 -->
  ...
  $('[data-toggle="tooltip"]').tooltip();
    ...
    $('a.add-line').on('click',function() {
     ...
     $('table.lines').on('click','a.delete-existent-line',function(){ 
        ...
        $('table tr[class="existent-line-' + xid + '"] .description').attr('name','deleted_description['+xid+']');
        $('table tr[class="existent-line-' + xid + '"] .qty').attr('name','deleted_quantity['+xid+']');
        $('table tr[class="existent-line-' + xid + '"] .price').attr('name','deleted_price['+xid+']');
        $('table tr[class="existent-line-' + xid + '"] .amount').attr('name','deleted_amount['+xid+']');
        $('table tr[class="existent-line-' + xid + '"]').css('display','none');
     ...
     $('table.lines').on('change',".price, .qty", function() {
         ...
         if (parseFloat(nqty)<0){
          nqty=-nqty;$('input[name="quantity['+ xid + ']"]').val(nqty);
         }
