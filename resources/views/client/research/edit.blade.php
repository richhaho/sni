...
@section('content')
  <div id="top-wrapper" >
    <div class="container-fluid">
      <div  class="col-xs-12">
          <h1 class="page-header">Edit Job Research
              <div class="pull-right">
                  
                  @if($work_order == '')
                  <a class="btn btn-danger " href="{{ route('client.jobs.edit', $job->id) }}"><i class="fa fa-times-circle"></i> Back</a>
                  @else
                  <a class="btn btn-danger " href="{{ route('workorders.edit',$work_order)}}"><i class="fa fa-times-circle"></i> Cancel</a>
                  @endif
                  <a href="{{ route('client.research.wizard.step1',$job->id)}}" class="btn btn-primary btn-property-search"><i class="fa fa-cogs"></i> Research Wizard</a>
                  <a href="#" data-toggle="modal" data-target="#modal-mark-complete-research" class="btn btn-info btn-property-search"><i class="fa fa-check"></i> Mark Research Completed</a>&nbsp;&nbsp;
                  <div class="col-xs-12">
                  </div>
                  &nbsp;
              </div>
          ...
          <div class="modal fade" id="modal-mark-complete-research" tabindex="-1" role="dialog">
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                  <h4 class="modal-title">Mark Research Completed</h4>
                </div>
                {!! Form::open(['route' => ['client.research.mark_completed',$job->id]]) !!}
                <div class="modal-body">
                  <div class="panel-body">
                    <div class="row">
                      <div class="col-md-12 form-group">
                          <label>Choose Work Order Status:</label><br>
                          {!!  Form::select('status',$markStatuses, $job->firstWorkorder()->status, ['class' => 'form-control']) !!}
                      </div>
                    </div>
                  </div>
                </div>
                <div class="modal-footer">
                  ...
                  <button class="btn btn-success btn-mark-completed" type="submit"><i class="fa fa-save"></i> Save</button>
                </div>
                {!! Form::close() !!}
              </div>
            </div>
     ...
     <!-- /.Attachments -->
      <div class="col-xs-12">
          <ul class="nav nav-tabs" role="tablist" id="job_tabs" data-tabs="tabs">
            <li role="presentation" class="active"><a onclick="clickTab('#jobform')" href="#jobform" aria-controls="messages" role="tab" data-toggle="tab">Job Info</a></li>
            <li role="presentation"><a onclick="clickTab('#parties')" href="#parties" aria-controls="messages" role="tab" data-toggle="tab">Job Parties</a></li>
            <!-- <li role="presentation"><a onclick="clickTab('#changes')" href="#changes" aria-controls="profile" role="tab" data-toggle="tab">Job Change Orders</a></li>
            <li role="presentation"><a onclick="clickTab('#payments')" href="#payments" aria-controls="messages" role="tab" data-toggle="tab">Job Payments</a></li>
            <li role="presentation"><a onclick="clickTab('#workorders')" href="#workorders" aria-controls="messages" role="tab" data-toggle="tab">Work Orders</a></li>
            <li role="presentation"><a onclick="clickTab('#questions')" href="#questions" aria-controls="messages" role="tab" data-toggle="tab">Additional Questions Summary</a></li> -->
            <li role="presentation"><a onclick="clickTab('#notes')" href="#notes" aria-controls="messages" role="tab" data-toggle="tab">Notes</a></li>
            <li role="presentation"><a onclick="clickTab('#attachments')" href="#attachments" aria-controls="attachments" role="tab" data-toggle="tab">Attachments</a></li>
          </ul>
          <!-- Nav tabs -->
          <div class="tab-content">
               <div role="tabpanel" class="tab-pane active" id="jobform">
                @include('client.research.jobform')
              </div>
              <div role="tabpanel" class="tab-pane" id="parties">
                  @include('client.research.components.jobparties-tab')
              </div>
              <!-- <div role="tabpanel" class="tab-pane" id="changes">
                  @include('client.research.changes.index', ['changes' => $job->changes()->orderBy('added_on','des')->get()])
              </div>
              <div role="tabpanel" class="tab-pane" id="payments">
                 @include('client.research.payments.index', ['payments' => $job->payments()->orderBy('payed_on','des')->get()])
              </div>
              <div role="tabpanel" class="tab-pane" id="workorders">
                @include('client.research.workorders.index',['works' => $job->workorders()->where('status', '!=', 'temporary')->get()])
              </div>
              <div role="tabpanel" class="tab-pane" id="questions">
                @include('client.research.workorders.question',['works' => $job->workorders()->where('status', '!=', 'temporary')->get()])
              </div> -->
               <div role="tabpanel" class="tab-pane " id="notes">
                 @include('client.notes.index', ['notes' => $job->notes()->where('deleted_at',null)->orderBy('entered_at','des')->get(),'e_name' => 'jobs','e_id' => $job->id])
              </div>
              <div role="tabpanel" class="tab-pane" id="attachments">
                @include('client.research.components.attachments-tab')
              </div>
          </div>
      </div>
     <!-- /.notes -->
...
$(function () {
  ...
  prefetch:  { url: '{{ route('list.countries') }}' , cache: false }
    ...
    $("#copy_job_id").select2({
        ...
        ajax: {
            url: '{{url("/client/jobs")}}/'+ job_id + '/copy',
            dataType: 'json',
            type: "GET",
            delay: 50,
   ...
   function formatJob (job) {
    ...
    $("#copy_job_id").on('change',function() {
        var job_id = $(this).val();
        $('.copy_data').load('{{url("/client/jobs")}}/' + job_id +'/copyform');
    });
...
function getInterestRate(client_id) {
    ...
    $.post('{{url("/client/clients")}}/' + client_id + '/interestrate', function( data ) {
         $( "#interest_rate" ).val( data );
    });
}

function getDefaultMaterials(client_id) {
    $.post('{{url("/client/clients")}}/' + client_id + '/defaultmaterials', function( data ) {
...
$('.full_address').keydown(function(e){
  ...
  if(e.keyCode==32 && full_address && full_address.indexOf(' ')<0){
    ...
    $.get('{{route("client.research.getaddress")}}?county='+county+'&address_1='+full_address , function( data ) {
