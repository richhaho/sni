...
<div id="page">
    ...
    <p class="text-justify">The undersigned lienor notifies you that: <br> The Lienor has furnished {!! $materials !!}  for the improvement of the real property identified as:</p>
    <table style="width: 100%">
        <tr>
            ...
            <td style="width: 50%; vertical-align: top;">
                ...
                @endif
                @if($noc)
                <span class="warning">NOC# {{$noc}}</span><br>
                @endif                
            </td>
        </tr>
    </table>     
    <p>
        ...
        @endif
        <br>
        The Lienor has been paid on account to date in the amount of ${{$amount_paid ? number_format($amount_paid,2):'________'}} for previously furnishing  {!! $materials ? $materials:'_______________' !!} for this improvement. 
        <br> 
        @if($continue_furnishing=='Yes') Lienor expects to continue furnishing  for this improvement in the future and the corresponding amount expected to become due is ${{$expected_amount_due ? number_format($expected_amount_due,2):'________'}}.
        @else
        The Lienor does not expect to furnish any additional materials or services under this contract as the last day of furnishing was {{ date_format(date_create($last_day_on_job ),'F jS, Y')}}. 
        @endif 
        <br> 
        ...
        Lienor was in privity with the Contractor.
        @else
        If the Lienor was not in privity with the Contractor then the Notice to Owner/Notice to Contractor was sent on {{$notice_date ? date_format(date_create($notice_date ),'F jS, Y'):'___________'}} by certified mail. @if ($certified_number) ({{$certified_number}}) @endif
        @endif
        <br>
    ...
    <br>
    @if($customer_working_for_contract!='Yes')
    The Lienor has also included the following document(s) to substantiate the amount claimed as unpaid in the notice:<br>
        ...
        <br>
    @endif
    @if($has_recorded_col=='Yes')
    In conjunction with this Notice of Nonpayment, the lienor has also recorded a Claim of Lien on this property.
    <br><br>
    @endif
    </div>
</div>