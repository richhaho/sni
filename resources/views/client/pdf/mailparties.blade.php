...
<div class="container">
    ...
    {!! Form::open(['route' => ['client.pdfpage.save',$work_order->id],'class'=>'save-page'])!!}
<div class="panel panel-default">
            ...
            <div class="panel-body form-horizontal party-fields{{$party->id}}">
                ...
                <div class="form-group email-group{{$party->id}} {{$errors->has('email[' .  $party->id . ']') ? '' : 'hidden'}}">
                    ...
                    @if ($party->mailing_type=='other-mail')
                         {!! Form::text('email[' .  $party->id . ']', old('email[' .  $party->id . ']',$party->email),['class'=>'form-control email-field noucase','id'=>'email-field'])!!}
                    @else
                        {!! Form::text('email[' .  $party->id . ']', old('email[' .  $party->id . ']',$party->email),['class'=>'form-control email-field noucase','id'=>'email-field', 'disabled'])!!}
                    @endif
                    </div>
                ...
                <div class="form-group number-group{{$party->id}} {{$party->mailing_type=='registered-mail' || $party->mailing_type=='express-mail' ? '' : 'hidden'}}">
                    <label class="col-xs-2">Number:</label>
                    <div class="col-xs-10">
                         {!! Form::text('mailing_number[' .  $party->id . ']', '',['class'=>'form-control mailing_number noucase','id'=>'mailing_number', 'readonly'=>true])!!}
                    </div>
                </div>
    ...
    <div class="panel-footer">
        <div class="pull-right">
            @if(count($parties)>0)<button type="submit" class="btn btn-success btn-save"><i class="fa fa-floppy-o"></i> Save</button>
            @endif
        ...
        $('.email-field').each(function() {
            ...
            if (!$(this).attr('disabled')) {;
                ...
                if (email.indexOf('@')<0 || email.indexOf('.')<0){
                    event.preventDefault();
                    $(this).parent().children('.email_validate').remove();
                    $(this).after( "<p class='email_validate'>Email type example@client.com is required.</p>" );
                    return;
                }
    ...
    $('.mailing_type').on('change',function() {
        ...
        if(xval == 'certified-green' || xval == 'certified-nongreen' ) {
            if($('.is-certified' + xid).length == 0) {
                ...
                xhtml += '<div class="form-group hidden"><label class="col-xs-2">&nbsp;</label><div class="col-xs-10"><div class="checkbox checkbox-slider--b-flat"><label><input name="return_receipt[' + xid + ']" type="checkbox" ><span>Return Receipt</span></label></div></div></div></div></div>';
