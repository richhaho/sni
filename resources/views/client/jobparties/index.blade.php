...
<style>
       ...
       .btn{ {{$style}} } 
       .btn-header{pointer-events:auto;opacity:1;}
</style>
@endsection
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <h1 class="page-header">{{$job->name}}<br><small>{{$job->client->company_name}}</small>
                            ...
                            @if($work_order == '')
                                <a class="btn btn-success  btn-header" href="{{ route('client.jobs.index') }}"><i class="fa fa-chevron-left"></i> Back to Jobs List</a>&nbsp;
                            @else
                                <a class="btn btn-success  btn-header" href="{{ route('client.notices.edit',$work_order) }}"><i class="fa fa-chevron-left"></i> Back to Work Order</a>&nbsp;
                            @endif
                             <a class="btn btn-primary  btn-header" href="{{ route('client.jobs.edit',$job->id) }}"> Edit Job</a>&nbsp;
                            <a class="btn btn-warning  btn-header" href="{{ route('client.notices.setfilter') . '?resetfilter=true&job_filter=' . $job->id }}"> Work Orders List</a>&nbsp;
                            <a class="btn btn-success btn-header" href="{{ route('wizard2.getjobworkorder') . '?job_id=' . $job->id }}"><i class="fa fa-plus"></i> New Work Order</a>&nbsp;
                            </div>
                        </h1>
                    ...
                    <div class="col-xs-12 message-box">
                        ...
                        @endif
                    <div class="row">
                        {!! Form::open (['route'=>['client.parties.store',$job->id],'files' => true]) !!}
                            ...
                            <div class="panel panel-default">
                                ...
                                <div class="panel-body">
                                    ...
                                    <div id="new-contact" class="{{ Session::exists('_old_input') ? array_key_exists("first_name",Session::get('_old_input')) ? '' : 'hidden' : 'hidden' }}">
                                        ...
                                        @include('client.jobparties.dynamicforms.newcontact')
                                    </div>
                                    ...
                                    <div class='row'>
                                        ...
                                        <div class="col-xs-12 form-group ">
                                            ...
                                            {!! Form::submit('Save',['class'=>'btn btn-success pull-right add_job_party']); !!}
                                       </div>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <div class="panel panel-default">
                                            ...
                                            <div class="panel-body">
                                               @foreach($job->parties()->ofType($type_key)->get() as $jobparty) 
                                                    ...
                                                    @include('client.jobparties.components.contacticon')
...
$(function () {
    ...
    $("#contact_id").select2({
        ...
        ajax: {
            url: '{{url("/client/jobs")}}/'+ job_id + '/contacts',
            dataType: 'json',
            type: "GET",
            delay: 50,
