<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ">
    ...
    @endif
        ...
        <div class="controls">
            @component('client.jobparties.components.deletemodal')
                @slot('id') 
                    {{ $jobparty->id }}
                @endslot
                @slot('job_party_name') 
            ...
            @endcomponent 
            @if($work_order =='') 
            <a href="{{ route('client.parties.edit',[ $jobparty->job_id, $jobparty->id])}}" class="btn btn-success btn-xs" role="button" data-toggle="tooltip" data-placement="right" title="Edit"><i class="fa fa-pencil"></i> Edit</a>
            @else
            <a href="{{ route('client.parties.edit',[ $jobparty->job_id, $jobparty->id]). '?workorder='. $work_order}}" class="btn btn-success btn-xs" role="button" data-toggle="tooltip" data-placement="right" title="Edit"><i class="fa fa-pencil"></i> Edit</a>
            @endif
            @component('client.jobparties.components.copyparty')
                @slot('id') 
                    {{ $jobparty->id }}
                @endslot
                @slot('job_id') 
