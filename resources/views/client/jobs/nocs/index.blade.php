...
<div class="row">
    <div class="col-xs-12" style="margin-bottom: 10px">
        ...
        @component('client.jobs.nocs.components.createmodal')
            @slot('job_id') 
                {{ $job->id }}
            @endslot
        @endcomponent
        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                <tr style="{{$noc->noc_number==$job->noc_number ? 'background-color: #FFE4E1' : ''}}">
                    ...
                    <td> 
                        @if($noc->copy_noc)
                        <a href="{{route('client.jobnocs.download', [$job->id, $noc->id])}}"><i class="fa fa-download"></i> Download</a>
                        @endif
                    </td>
                    ...
                    <td>
                        ...
                        @else
                        <a href="{{route('client.jobnocs.setcurrent', [$job->id, $noc->id])}}" class="btn btn-warning">Set to Current NOC</a>
                        @endif
                    </td>
                    <td>
                        <div class="btn-group pull-right dropup">
                            ...
                            @component('client.jobs.nocs.components.editmodal')
                                @slot('id') 
                                    {{ $noc->id }}
                                @endslot
                                @slot('job_id') 
                                ...
                                @endslot
                            @endcomponent
                            @component('client.jobs.nocs.components.deletemodal')
                                @slot('id') 
                                    {{ $noc->id }}
                                @endslot
                                @slot('job_id') 
